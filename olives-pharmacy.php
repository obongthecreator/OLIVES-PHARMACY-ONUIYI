...
.olives-btn{display:inline-flex;align-items:center;justify-content:center;padding:.5rem 1rem;border-radius:9999px;font-weight:600;line-height:1.1}
.olives-btn-add{background:linear-gradient(135deg,#4ade80,#16a34a)!important;color:#fff!important;border:1px solid #15803d!important}
.olives-btn-edit{background:linear-gradient(135deg,#fde68a,#facc15)!important;color:#713f12!important;border:1px solid #ca8a04!important}
.olives-btn-delete{background:linear-gradient(135deg,#fda4af,#dc2626)!important;color:#fff!important;border:1px solid #b91c1c!important}
...
<a data-olives-nav-key="<?php echo esc_attr( $link['key'] ); ?>" class="px-4 py-2 rounded-full border transition-all duration-200 <?php echo $link['active'] ? 'bg-green-100 text-olives-green border-olives-green font-semibold' : 'bg-white text-olives-green border-green-200 hover:border-olives-green hover:bg-green-50'; ?>" href="<?php echo esc_url( $link['url'] ); ?>"><?php echo esc_html( $link['label'] ); ?></a>
<?php endforeach; ?>
</div>
<?php endif; ?>
</div>
...
$html .= '<section class="olives-card p-4 mb-4"><div class="flex flex-col md:flex-row md:items-center gap-3 md:justify-between"><div class="relative w-full md:max-w-md"><iconify-icon icon="solar:magnifer-linear" class="absolute left-3 top-3 text-slate-400"></iconify-icon><input id="olives-stock-search" class="w-full pl-10 pr-3 py-2 rounded-[20px] border-slate-200" placeholder="Search products..." /></div><div class="flex gap-2"><button id="olives-add-product" class="olives-btn olives-btn-add">Add Product</button><button id="olives-export-products" class="px-4 py-2 rounded-[20px] border">Export CSV</button></div></div><div class="olives-table-wrap mt-4"><table class="w-full text-sm"><thead><tr class="text-left text-slate-500"><th class="p-2">Name</th><th class="p-2">NAFDAC</th><th class="p-2">Batch</th><th class="p-2">Expiry</th><th class="p-2">Qty</th><th class="p-2">Imported Today</th><th class="p-2">Cost</th><th class="p-2">Selling</th><th class="p-2">Reorder</th><th class="p-2">Actions</th></tr></thead><tbody id="olives-stock-tbody"></tbody></table></div></section>';
$html .= '<section class="olives-card p-4 mb-4"><div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3"><input type="hidden" id="olives-user-id" /><div><label class="text-sm font-medium">Username</label><input id="olives-user-username" class="w-full mt-1 rounded-[20px] border-slate-200" /></div><div><label class="text-sm font-medium">Display Name</label><input id="olives-user-display" class="w-full mt-1 rounded-[20px] border-slate-200" /></div><div><label class="text-sm font-medium">Email</label><input id="olives-user-email" type="email" class="w-full mt-1 rounded-[20px] border-slate-200" /></div><div><label class="text-sm font-medium">Role</label><select id="olives-user-role" class="w-full mt-1 rounded-[20px] border-slate-200"><option value="administrator">Administrator</option><option value="editor">Editor</option><option value="author">Author</option><option value="shop_manager">Shop Manager</option><option value="subscriber">Subscriber</option></select></div><div><label class="text-sm font-medium">Password</label><input id="olives-user-password" type="password" class="w-full mt-1 rounded-[20px] border-slate-200" placeholder="Required for new user" /></div><div class="flex items-end gap-2"><button id="olives-user-save" class="olives-btn olives-btn-add">Save User</button><button id="olives-user-reset" class="px-4 py-2 rounded-[20px] border">Reset</button></div></div></section>';
  ...
  const initAdmin = async () => {
    ...
    const loadUsers = async () => {
      ...
      try {
        ...
        tbody.innerHTML = users.map(u=>`<tr class="border-b"><td class="p-2">${u.username}</td><td class="p-2">${u.display_name}</td><td class="p-2">${u.email}</td><td class="p-2">${u.role || '-'}</td><td class="p-2 text-xs">${u.capabilities || '-'}</td><td class="p-2"><div class="flex flex-wrap gap-2"><button class="olives-btn olives-btn-edit" data-user-edit="${u.id}">Edit</button><button class="olives-btn olives-btn-delete" data-user-del="${u.id}">Delete</button></div></td></tr>`).join('') || '<tr><td colspan="6" class="p-2 text-slate-500">No users found.</td></tr>';
